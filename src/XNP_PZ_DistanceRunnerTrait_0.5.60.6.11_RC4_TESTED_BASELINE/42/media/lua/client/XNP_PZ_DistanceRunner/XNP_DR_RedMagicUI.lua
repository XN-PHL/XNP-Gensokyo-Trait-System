...
function Panel:onMouseMove(dx, dy)
    ...
    if not RedUI.hasTrait then
        ...
        detail = getText("UI_XNPMarker_RedHelp") .. "<LINE>" .. getText("UI_XNPMarker_RedMode") .. ": " .. tostring(RedUI.mode) .. "<LINE>" .. getText("UI_XNPMarker_RedCount") .. ": " .. tostring(RedUI.count)
    end
...
local function tooltipText()
    ...
    return getText("UI_XNPMarker_RedHelp") .. "<LINE>" .. getText("UI_XNPMarker_RedMode") .. ": " .. tostring(RedUI.mode) .. "<LINE>" .. getText("UI_XNPMarker_RedCount") .. ": " .. tostring(RedUI.count)
end
...
function Panel:onRightMouseUp(x, y)
    ...
    if armed and RedUI.desiredVisible and not RedUI.mapHidden and not Drag.IsDragging(self)
        ...
        local oldMode = Core.RedGuardianMark.GetMode(RedUI.player)
        local newMode = oldMode == Core.RedGuardianMark.MODE_STAMINA
            and Core.RedGuardianMark.MODE_TREATMENT or Core.RedGuardianMark.MODE_STAMINA
        local changed = Core.RedGuardianMark.SetMode(RedUI.player, newMode)
        if changed then
            RedUI.MarkDirty()
            Core.Audio.PlayOnce(RedUI.player, "MARKER_TOGGLE", "red-toggle:" .. tostring(Frame.NowMs()))
        end
        return changed == true
    end
    return armed
