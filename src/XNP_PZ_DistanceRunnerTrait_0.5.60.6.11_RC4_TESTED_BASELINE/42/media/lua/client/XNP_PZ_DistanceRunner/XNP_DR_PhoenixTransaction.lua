...
local function validateRecovery(player, snapshot, config)
    ...
    if config.clearZombification and (type(body.setInfected) ~= "function"
        or type(body.setIsFakeInfected) ~= "function") then
    ...
    return true, { body = body, endurance = config.enduranceRestore }
end
...
local function commitRecovery(player, contract, config)
    local ok, err = pcall(function()
        ...
        if config.clearZombification then
    ...
    if not living or not snapshot or snapshot.directHealth < 0.95 or snapshot.bodyHealth < 0.95 then
