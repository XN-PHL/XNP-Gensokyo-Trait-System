...
function Tooltip.Show(owner, name, description)
    if Core.SandboxTuning and (not Core.SandboxTuning.GetBoolean("EnableTooltips", true)
        or not Core.SandboxTuning.GetBoolean("TooltipEnabled", true)) then
        return Tooltip.Hide("SANDBOX_DISABLED")
    end
