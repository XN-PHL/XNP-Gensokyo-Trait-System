...
function Config.Refresh()
    ...
    if hash ~= Config.lastHash then
        ...
        print(string.format("[XNP PHOENIX CONFIG] enabled=%s trigger=%.2f invulnerable_sec=%.1f cooldown_mode=%s cooldown_real_seconds=%.1f", tostring(value.enabled), value.triggerHealth, value.invulnerabilitySeconds, value.cooldownMode, value.cooldownRealSeconds))
    end
    return value
end
