...
function Classifier.IsValidAttacker(attacker)
    ...
    if call(attacker, "getVehicle") ~= nil
        and Core.SandboxTuning.GetBoolean("GreenVehicleExcluded", true) then
        return false, "VEHICLE_IMPACT_EXCLUDED"
    end
...
function Classifier.IsVerifiedMeleeWeapon(attacker, weapon)
    ...
    if call(weapon, "isRanged") ~= false
        and Core.SandboxTuning.GetBoolean("GreenFirearmExcluded", true) then
        return false, "RANGED_OR_UNVERIFIED_EXCLUDED"
    end
    if call(attacker, "isDoShove") == true
        and Core.SandboxTuning.GetBoolean("GreenShoveExcluded", true) then
        return false, "PUSH_SHOVE_EXCLUDED"
    end
    if call(attacker, "isPerformingGrappleGrabAnimation") == true then return false, "GRAPPLE_EXCLUDED" end
    if call(attacker, "isAimAtFloor") == true and call(attacker, "isDoShove") == true
        and Core.SandboxTuning.GetBoolean("GreenStompExcluded", true) then
        return false, "STOMP_EXCLUDED"
    end
    if call(weapon, "getType") == "BareHands"
        and Core.SandboxTuning.GetBoolean("GreenBareHandsExcluded", true) then
        return false, "UNARMED_EXCLUDED"
    end
    if call(weapon, "getPhysicsObject") ~= nil
        and Core.SandboxTuning.GetBoolean("GreenExplosionExcluded", true) then
        return false, "THROWN_PROJECTILE_EXCLUDED"
    end
    if call(weapon, "isAimedFirearm") == true
        and Core.SandboxTuning.GetBoolean("GreenFirearmExcluded", true) then
        return false, "FIREARM_EXCLUDED"
    end
