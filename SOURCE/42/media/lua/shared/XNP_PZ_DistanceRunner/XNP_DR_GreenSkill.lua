...
function GreenSkill.IsEnabled(player)
    ...
    if Core.SandboxTuning.GetBoolean("GreenMeleeEnabled", true) ~= true then return false end
...
function GreenSkill.GetTier(player)
    ...
    if not GreenSkill.IsEnabled(player) then
        return { name = "WHITE",
            multiplier = Core.SandboxTuning.GetNumber("GreenDisabledDamageMultiplier", 1.0, 1.0, 20.0),
            enduranceCost = 0.0, enabled = false, endurance = value }
    end
    if value == nil then
