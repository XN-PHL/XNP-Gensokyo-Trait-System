...
local function playOnceInternal(player, soundKey, eventToken, volumePercent)
    ...
    if volume < 1 and handle ~= nil then
        ...
        if emitterOk and emitter and type(emitter.setVolume) == "function" then
            pcall(function() emitter:setVolume(handle, volume) end)
        end
    end
    print("[XNP AUDIO] one_shot=true sound=" .. soundName .. " event=" .. tostring(eventToken)
        .. " requested_volume_percent=" .. tostring(volumePercent or 100))
    return true, handle
end
...
function Audio.PlayOnce(player, soundKey, eventToken)
    ...
    return playOnceInternal(player, soundKey, eventToken, 100)
end

-- Green casts use their admission-time sound snapshot in the update loop.
function Audio.PlayOnceConfigured(player, soundKey, eventToken, enabled, volumePercent)
    if enabled ~= true then return false, "SOUNDS_DISABLED_IN_CAST_SNAPSHOT" end
    return playOnceInternal(player, soundKey, eventToken, volumePercent)
end
