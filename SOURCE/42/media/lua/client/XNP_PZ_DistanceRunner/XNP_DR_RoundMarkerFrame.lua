...
local function refreshGeometry()
    local tuning = XNP_PZ_DistanceRunner.SandboxTuning
    if not tuning then return end
    local scale = tuning.GetNumber("MarkerScale", 1.0, 0.5, 2.0)
    Frame.PANEL_SIZE = math.max(16, math.floor(32 * scale + 0.5))
    Frame.CENTER_SIZE = math.max(8, math.floor(16 * scale + 0.5))
    Frame.CENTER_OFFSET = math.floor((Frame.PANEL_SIZE - Frame.CENTER_SIZE) / 2)
    Frame.GAP = math.floor(tuning.GetNumber("MarkerSpacing", 4, 0, 32) + 0.5)
    Frame.UPDATE_INTERVAL_MS = tuning.GetNumber("MarkerUpdateIntervalFrames", 15, 1, 120) * (1000 / 60)
end

local function opacity()
    local tuning = XNP_PZ_DistanceRunner.SandboxTuning
    return tuning and tuning.GetNumber("MarkerOpacity", 1.0, 0.1, 1.0) or 1.0
end
...
function Frame.LoadPosition(player, newKey, legacyKeys, defaultRightIndex)
    ...
    local tuning = XNP_PZ_DistanceRunner.SandboxTuning
    local persistent = not tuning or tuning.GetBoolean("MarkerPositionPersistenceEnabled", true)
    local reset = tuning and tuning.GetBoolean("MarkerResetPositions", false)
    if not persistent or reset then
        if reset then data[newKey] = nil end
        return defaultX, defaultY, reset and "SANDBOX_RESET" or "PERSISTENCE_DISABLED"
    end
...
function Frame.Draw(panel, centerTexture, colorName)
    ...
    panel:drawTextureScaled(Frame.shellSolid, 0, 0, Frame.PANEL_SIZE, Frame.PANEL_SIZE,
        color.a * opacity(), color.r, color.g, color.b)
